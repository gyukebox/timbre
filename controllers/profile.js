...
exports.editProfileImage = (req, res) => {
  if (req.session.user === null || req.session.user === undefined) {
    ...
    res.status(403).send('프로필을 변경할 수 있는 권한이 없습니다.');
  } else {
    userModel
      ...
      .then((user) => {
        if (user === null) {
          ...
          user
            ...
            .then(() => {
              res.status(204).send('프로필을 성공적으로 변경했습니다.');
            });
        }
      });
  }
...
exports.editIntroduction = (req, res) => {
  if (req.body.introduction === undefined) {
    res.status(412).send('파라미터가 부족합니다');
  } else if (req.session.user === null || req.session.user === undefined) {
    res.status(401).send('로그인이 필요합니다.');
  } else if (req.session.user.userId.toString() !== req.params.id) {
    res.status(403).send('본인의 소개 내용만 변경 가능합니다');
  } else {
    userModel
      .findOne({
        where: { userId: req.params.id },
      ...
      .then((user) => {
        user
          ...
          .then(() => {
            res.status(204).send('소개를 변경하는데 성공했습니다');
          });
      })
      .catch((err) => {
        res.status(400).json({
